  ...
  try {
    ...
    if (type === 'deposit') {
      ...
      if (tx.collateralBTC && Number(tx.collateralBTC) > 0) {
        // Treat collateral deposits as both collateral value and available wallet balance
      ...
      try {
        ...
        if (isMembershipTx && isCompleted && created.userId) {
          ...
          if (user) {
            ...
            emitToUser(created.userId, 'user:updated', { id: user._id, isMember: user.isMember, membershipPaidAmount: user.membershipPaidAmount, membershipPaidAt: user.membershipPaidAt, membershipExpiresAt: user.membershipExpiresAt });
          }
        }
      } catch (e) {
