      ...
      if (isCompleted) {
        ...
        if (tx.userId) {
          ...
          if (user) {
            ...
            await user.save();
            try { const { emitToUser } = require('../services/socketService'); if (tx.userId) emitToUser(tx.userId, 'user:updated', { id: user._id, savingsBalanceUSD: user.savingsBalanceUSD, collateralBalanceUSD: user.collateralBalanceUSD, isMember: user.isMember }); } catch(e){}
          }
        }
      }
